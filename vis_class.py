...
def crop_img2model(
    ...
    for i in tqdm.tqdm(range(crop_size // 2, x - crop_size // 2 + 1, crop_size)):
        for j in range(crop_size // 2, y - crop_size // 2 + 1, crop_size):
            ...
            with torch.no_grad():
                ...
                out = model(crop_hsi[None], crop_lidar[None])
        
            out = out.cpu().numpy()
            out = np.argmax(out[0], 0)
            outs[i - crop_size // 2: i + crop_size // 2, j - crop_size // 2: j + crop_size // 2] = out
    ...
    pic_out = seg_out[crop_size // 2: -crop_size // 2, crop_size // 2: -crop_size // 2]
...
def main(cfg: DictConfig):
    ...
    pic_out = crop_img2model(model, device, hsi, lidar, crop_size)
    ...
    Image.fromarray(pic_out.astype(np.uint8)).save(out_path+'/'+ 'classify'+'.png')
