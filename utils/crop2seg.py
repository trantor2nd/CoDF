...
def crop_data(img1, img2, Trlabel,Tslabel, path_out,crop_size):
    ...
    ii = 0
    #train dataset
    for i in tqdm.tqdm(range(crop_size // 2, x + crop_size // 2,crop_size//4)):
        for j in range(crop_size // 2, y + crop_size // 2,crop_size//4):
            if Trlabel[i - crop_size // 2: i + crop_size // 2+crop_size%2, j - crop_size // 2: j + crop_size // 2+crop_size%2].sum() == 0:
                continue
            path = path_out+ '/train'
            ...
            savemat(path + '/' + str(ii) + '.mat', {
                ...
                'label': Trlabel[i - crop_size // 2: i + crop_size // 2,
                             j - crop_size // 2: j + crop_size // 2].astype(np.int8) - 1, # first class : 0
            })
            ii += 1
    ...
    for i in tqdm.tqdm(range(crop_size // 2, x + crop_size // 2,crop_size//4)):
        for j in range(crop_size // 2, y + crop_size // 2,crop_size//4):
            if Tslabel[i - crop_size // 2: i + crop_size // 2+crop_size%2, j - crop_size // 2: j + crop_size // 2+crop_size%2].sum() == 0:
                continue
            path = path_out+ '/test'
            os.makedirs(path, exist_ok=True)
            savemat(path + '/' + str(ii) + '.mat', {
                'hsi': img1[i - crop_size // 2: i + crop_size // 2+crop_size%2, j - crop_size // 2: j + crop_size // 2+crop_size%2],
                'lidar': img2[i - crop_size // 2: i + crop_size // 2+crop_size%2, j - crop_size // 2: j + crop_size // 2+crop_size%2],
                'label':  Tslabel[i - crop_size // 2: i + crop_size // 2,
                             j - crop_size // 2: j + crop_size // 2].astype(np.int8) - 1, # first class : 0
            })
            ii += 1
...
def crop_Houston2013(crop_size):
    ...
    path_out = '/home/data/shared_datasets/Houston2013/'+'{}x{}forSeg'.format(crop_size,crop_size)
...
def crop_MUUFL(crop_size):
    ...
    path_out = f'/home/data/shared_datasets/MUUFL/' + '{}x{}forSeg'.format(crop_size,crop_size)
    os.makedirs(path_out, exist_ok=True)
    crop_data(hsi, lidar, train_label,test_label, path_out,crop_size )
...
def crop_Augsburg(crop_size):
    ...
    path_out = '/home/data/shared_datasets/Augsburg/' + '{}x{}forSeg'.format(crop_size,crop_size)
    os.makedirs(path_out, exist_ok=True)
...
def crop_Trento(crop_size):
    ...
    path_out = f'/home/data/shared_datasets/Trento/' + '{}x{}forSeg'.format(crop_size,crop_size)
    os.makedirs(path_out, exist_ok=True)
    crop_data(hsi, lidar, train_label,test_label, path_out,crop_size )
